...
//        std::cout << "tool_external_wrench_force_z " << feedback.base().tool_external_wrench_force_z() << " N" << std::endl;
    ...
    pose->set_y(feedback.base().tool_pose_y() + 0.3);          // y (meters)
    pose->set_z(feedback.base().tool_pose_z() - 0.2);          // z (meters)
    pose->set_theta_x(feedback.base().tool_pose_theta_x() + 90);    // theta x (degrees)
...
//    std::cout << "tool_external_wrench_force_z " << feedback.base().tool_external_wrench_force_z() << " N" << std::endl;
    ...
    std::cout << "Waiting for movement to finish ..." << std::endl;
    timer_count = 0;
    TIME_DURATION = 90.0f; // Duration of the example (seconds)
//    while (timer_count < (TIME_DURATION)) {
//        auto feedback = base_cyclic->RefreshFeedback();
//        std::cout << "tool_external_wrench_force_z " << feedback.base().tool_external_wrench_force_z() << " N" << std::endl;
//        timer_count++;
...
bool example_cartesian_action_movement_vertical(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic, float displacement, int &timer_count)
{
    std::cout << "Starting Cartesian action movement ..." << std::endl;
    ...
    // Real-time loop
//    int timer_count = 0;
//    float TIME_DURATION = timer_count + 30.0f; // Duration of the example (seconds)
//    while (timer_count < (TIME_DURATION)) {
//        auto feedback = base_cyclic->RefreshFeedback();
////        std::cout << "tool_external_wrench_force_z " << feedback.base().tool_external_wrench_force_z() << " N" << std::endl;
//        timer_count++;
//    }
    ...
    pose->set_z(feedback.base().tool_pose_z() + displacement);          // z (meters)
...
//    timer_count = 0;
    float TIME_DURATION = timer_count + 90.0f; // Duration of the example (seconds)
    while (timer_count < (TIME_DURATION)) {
        auto feedback = base_cyclic->RefreshFeedback();
...
int main(int argc, char **argv)
{
    ...
    file.open("kinova_forces.txt"); //, std::ios_base::app);
...
//    success &= example_cartesian_action_movement(base, base_cyclic);
    success &= example_cartesian_action_movement_vertical(base, base_cyclic, -0.045, timer_count); // translating down
    timer_count = 90;
    success &= example_cartesian_action_movement_vertical(base, base_cyclic, 0.045, timer_count);  // translating up
...
//    int TIME_DURATION = 200.0f; // Duration of the example (seconds)
    int TIME_DURATION = 0.0f; // Duration of the example (seconds)
