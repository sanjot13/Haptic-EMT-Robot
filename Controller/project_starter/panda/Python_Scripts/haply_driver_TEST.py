...
class Handle(HaplyHardwareAPI.Handle):
    ...
    def OnReceiveHandleStatusMessage(self, device_id, quaternion, error_flag, hall_effect_sensor_level, user_data_length, user_data):
        ...
        if self.previous == 0 and self.current == 1: # detect rising edge
            self.Rising_Edge_Detected = True
        if self.Rising_Edge_Detected == True:
            self.tick_count += 1 # start counting
        if self.previous == 1 and self.current == 0: # detect falling edge
            self.Rising_Edge_Detected = False     

        if self.Rising_Edge_Detected == False and self.tick_count > 0 and self.tick_count < 50:
            self.Hold_Position = not self.Hold_Position
            self.target_position = position
            self.tick_count = 0 # reset counter back to 0
        elif self.Rising_Edge_Detected == True and self.tick_count >= 50:
            print("Hold Detected")
        elif self.Rising_Edge_Detected == False and self.tick_count >= 50:
            self.tick_count = 0
