...
// specify whether to hold end effector position
    ...
    while (runloop) {
		...
		if(controller_number = 1){
			...
			// this if case controls the end effector position in task space
				...
				// switch to force control if we reach desired position and orientation
				if (pose_task->goalPositionReached(0.015) && pose_task->goalOrientationReached(0.03)){
					...
					// after 2 seconds of reaching goal position, switch to drop state
					if (time - initial_time > 2){

						// disable velocity saturation of operational point
						pose_task->disableVelocitySaturation();

						// TESTING->lowering gains when going into drop state
				...
				// manually changing desired position to lower ee at a speed of 0.007 m/s
