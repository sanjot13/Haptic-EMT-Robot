...
while True:
    ...
    # remove x and y components of forces
    force[0:2] = 0
    print(force)
    ...
    if force[2] > 10:
        force[2] = 10
        print("z force safety limit exceeded, setting to 20")
        print(force)

    # flip axes before sending
    if handle.Hold_Position == False:
        position, velocity = inverse3.end_effector_force(force)
        # print("position: {}".format(position))
    elif handle.Hold_Position == True:
        position, velocity = inverse3.end_effector_position(handle.target_position)
    kinova_position = [(-position[1] + calibration_position[1])*position_scaling_factor, (position[0] + calibration_position[0])*position_scaling_factor, (position[2] + calibration_position[2])]
